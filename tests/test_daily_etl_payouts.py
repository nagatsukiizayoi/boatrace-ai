...
def test_daily_etl_ignores_cancelled_groups_for_unscheduled_races(monkeypatch,tmp_path): program,result=make_daily_frames(); payouts=make_payouts(); bet_types=["WIN","PLACE","EXACTA","QUINELLA","QUINELLA_PLACE","TRIFECTA","TRIO"]; extras=pd.DataFrame([{"race_date":"2026-06-30","venue_code":"13","race_no":race_no,"bet_type":bet_type,"selection_no":1,"combination":pd.NA,"payout_yen":pd.NA,"popularity":pd.NA,"payout_status":"CANCELLED","source_kind":"SUMMARY"} for race_no in range(1,13) for bet_type in bet_types]); install_mocks(monkeypatch,program,result,pd.concat([payouts,extras],ignore_index=True)); outcome=daily_etl.process_daily_files(tmp_path/"B260630.TXT",tmp_path/"K260630.TXT","2026-06-30",tmp_path); written=pd.read_parquet(outcome["paths"]["payout"]); quality=outcome["quality"]; assert quality["status"]=="SUCCESS"; assert len(written)==7; assert set(written["venue_code"])=={"01"}; assert not written["payout_status"].eq("CANCELLED").any(); assert quality["payout_unexpected_group_count"]==0; assert quality["payout_summary_mismatch"]==0
