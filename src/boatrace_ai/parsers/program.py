...
PROGRAM_PATTERN=re.compile(r"^(?P<boat_no>[1-6])\s(?P<racer_id>\d{4})(?P<racer_name>.{4})(?P<age>\d{2})(?P<branch>.{2})(?P<weight_kg>\d{2})(?P<class>[AB][12])\s*(?P<national_win_rate>\d\.\d{2})\s*(?P<national_place2_rate_pct>\d{1,3}\.\d{2})\s*(?P<local_win_rate>\d\.\d{2})\s*(?P<local_place2_rate_pct>\d{1,3}\.\d{2})\s*(?P<motor_no>\d{1,2})\s*(?P<motor_place2_rate_pct>\d{1,3}\.\d{2})\s*(?P<boat_no_equipment>(?:1\d{2}|\d{1,2}))\s*(?P<boat_place2_rate_pct>(?:100\.00|\d{1,2}\.\d{2}))(?P<series_results_raw>.*)$")
