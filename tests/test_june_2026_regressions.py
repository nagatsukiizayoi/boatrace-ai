...
def test_complete_result_absence_is_cancelled(monkeypatch,tmp_path): program,result=make_daily_frames([]); monkeypatch.setattr(daily_etl,"parse_program_file",lambda *args,**kwargs:program.copy()); monkeypatch.setattr(daily_etl,"parse_result_file",lambda *args,**kwargs:result.copy()); monkeypatch.setattr(daily_etl,"parse_payout_file",lambda *args,**kwargs:make_cancelled_payouts()); outcome=daily_etl.process_daily_files(tmp_path/"B260627.TXT",tmp_path/"K260627.TXT","2026-06-27",tmp_path); quality=outcome["quality"]; assert quality["status"]=="SUCCESS"; assert quality["left_only"]==6; assert quality["right_only"]==0; assert quality["partial_result_missing"]==0; assert quality["cancelled_entry_count"]==6; assert quality["cancelled_race_count"]==1; assert quality["finish_status_counts"]["CANCELLED"]==6; assert quality["cancelled_races"]==[{"race_date":"2026-06-27","venue_code":"04","race_no":1}]; merged=pd.read_parquet(outcome["paths"]["merged"]); assert len(merged)==6; assert not merged["result_available"].any(); assert merged["race_cancelled"].all()

def test_partial_result_absence_is_error(monkeypatch,tmp_path): program,result=make_daily_frames([1,2,3,4,5]); monkeypatch.setattr(daily_etl,"parse_program_file",lambda *args,**kwargs:program.copy()); monkeypatch.setattr(daily_etl,"parse_result_file",lambda *args,**kwargs:result.copy()); monkeypatch.setattr(daily_etl,"parse_payout_file",lambda *args,**kwargs:make_cancelled_payouts()); captured=pytest.raises(daily_etl.DailyETLError,daily_etl.process_daily_files,tmp_path/"B260627.TXT",tmp_path/"K260627.TXT","2026-06-27",tmp_path); assert "partial_result_missing=1" in str(captured.value)


def make_cancelled_payouts():
    bet_types=["WIN","PLACE","EXACTA","QUINELLA","QUINELLA_PLACE","TRIFECTA","TRIO"]
    return pd.DataFrame([{"race_date":"2026-06-27","venue_code":"04","race_no":1,"bet_type":bet_type,"selection_no":1,"combination":pd.NA,"payout_yen":pd.NA,"popularity":pd.NA,"payout_status":"CANCELLED","source_kind":"SUMMARY"} for bet_type in bet_types])
